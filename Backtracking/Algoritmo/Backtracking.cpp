...
Backtracking::Resultado Backtracking::backtracking_con_poda(Caso caso) {
    ...
    while (nivel != -1 && !is_best_solucion) {
        ...
        if (!is_best_solucion) {
            ...
            if (criterio_con_poda(dinero_gastado, nivel, voa, caso)) nivel++;
            else {
...
bool Backtracking::criterio_con_poda(const int dinero_gastado, const int nivel, const int voa, Caso &caso) {
